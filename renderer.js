...
let quickCaptureSettings = {
  enabled: true,
  globalHotkey: 'CommandOrControl+Q',
  windowStyle: 'floating',
  clearAfterSend: true,
  showFilenamePreview: true,
  vaults: []
};

let editingVault = null; // Track which vault is being edited
let selectedIcon = '📓'; // Currently selected icon in picker

// Open Quick Capture Settings Dialog
ipcRenderer.on('open-quick-capture-settings', () => {
  // Request current settings from main process
  ipcRenderer.send('get-quick-capture-settings');
});

// Receive settings from main process
ipcRenderer.on('quick-capture-settings', (event, settings) => {
  quickCaptureSettings = settings;
  openQuickCaptureSettingsDialog();
});

function openQuickCaptureSettingsDialog() {
  const dialog = document.getElementById('quick-capture-settings-dialog');

  // Populate settings
  document.getElementById('quick-capture-enabled').checked = quickCaptureSettings.enabled;
  document.getElementById('quick-capture-hotkey').value = quickCaptureSettings.globalHotkey;

  const windowStyleRadios = document.querySelectorAll('input[name="window-style"]');
  windowStyleRadios.forEach(radio => {
    radio.checked = radio.value === quickCaptureSettings.windowStyle;
  });

  document.getElementById('clear-after-send').checked = quickCaptureSettings.clearAfterSend;
  document.getElementById('show-filename-preview').checked = quickCaptureSettings.showFilenamePreview;

  // Populate vaults list
  renderVaultsList();

  dialog.classList.remove('hidden');
}

function closeQuickCaptureSettingsDialog() {
  const dialog = document.getElementById('quick-capture-settings-dialog');
  dialog.classList.add('hidden');
}
...
function renderVaultsList() {
  ...
  if (quickCaptureSettings.vaults.length === 0) {
    container.innerHTML = `
      <div class="vaults-empty">
        <p>No vaults configured</p>
        <p class="hint">Click "+ Add Vault" to configure your first destination</p>
      </div>
    `;
    return;
  }
  ...
  quickCaptureSettings.vaults.forEach(vault => {
    const item = document.createElement('div');
    item.className = 'vault-item';
    item.innerHTML = `
      <div class="vault-item-icon">${vault.icon || '📓'}</div>
      <div class="vault-item-info">
        <div class="vault-item-name">${vault.name}</div>
        <div class="vault-item-details">${vault.path} → ${vault.inboxFolder}</div>
      </div>
      <div class="vault-item-hotkey">${vault.hotkey}</div>
      <div class="vault-item-actions">
        <button class="edit-vault-btn" data-vault-id="${vault.id}">Edit</button>
        <button class="delete-btn delete-vault-btn" data-vault-id="${vault.id}">Delete</button>
      </div>
    `;
  ...
  container.querySelectorAll('.edit-vault-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const vault = quickCaptureSettings.vaults.find(v => v.id === vaultId);
      if (vault) {
        editVault(vault);
      }
    });
...
// Open Vault Edit Dialog
function openVaultEditDialog() {
  const dialog = document.getElementById('vault-edit-dialog');
  const title = document.getElementById('vault-edit-title');

  if (editingVault) {
    title.textContent = 'Edit Vault';
    // Populate with existing data
    document.getElementById('vault-name').value = editingVault.name;
    document.getElementById('vault-path').value = editingVault.path;
    document.getElementById('vault-inbox').value = editingVault.inboxFolder;
    selectedIcon = editingVault.icon || '📓';
    document.getElementById('selected-icon-btn').textContent = selectedIcon;
    document.getElementById('vault-hotkey').value = editingVault.hotkey;

    // Set filename pattern
    const pattern = editingVault.filenamePattern;
    if (pattern === '{{date}}-{{title}}.md') {
      document.querySelector('input[name="filename-pattern"][value="date-title"]').checked = true;
    } else if (pattern === '{{title}}.md') {
      document.querySelector('input[name="filename-pattern"][value="title"]').checked = true;
    } else {
      document.querySelector('input[name="filename-pattern"][value="custom"]').checked = true;
      document.getElementById('custom-filename-pattern').value = pattern;
    }

    // Set file mode
    const fileModeRadios = document.querySelectorAll('input[name="file-mode"]');
    fileModeRadios.forEach(radio => {
      radio.checked = (radio.value === 'append' && editingVault.appendMode) ||
                     (radio.value === 'new' && !editingVault.appendMode);
    });
  } else {
    ...
    document.getElementById('vault-inbox').value = 'inbox';
    selectedIcon = '📓';
    document.getElementById('selected-icon-btn').textContent = selectedIcon;
    document.getElementById('vault-hotkey').value = 'Ctrl+1';
    document.querySelector('input[name="filename-pattern"][value="date-title"]').checked = true;
    document.querySelector('input[name="file-mode"][value="new"]').checked = true;
  }

  dialog.classList.remove('hidden');
}

function closeVaultEditDialog() {
  const dialog = document.getElementById('vault-edit-dialog');
  dialog.classList.add('hidden');
}
...
function deleteVault(vaultId) {
  ...
  if (confirm(`Delete vault "${vault.name}"?`)) {
    quickCaptureSettings.vaults = quickCaptureSettings.vaults.filter(v => v.id !== vaultId);
    renderVaultsList();
  }
}
...
document.getElementById('save-vault-btn').addEventListener('click', () => {
  ...
  const inbox = document.getElementById('vault-inbox').value.trim();
  const hotkey = document.getElementById('vault-hotkey').value;
  ...
  const patternRadio = document.querySelector('input[name="filename-pattern"]:checked');
  let filenamePattern = '{{date}}-{{title}}.md';
  if (patternRadio.value === 'date-title') {
    filenamePattern = '{{date}}-{{title}}.md';
  } else if (patternRadio.value === 'title') {
    filenamePattern = '{{title}}.md';
  } else {
    filenamePattern = document.getElementById('custom-filename-pattern').value.trim() || '{{title}}.md';
  }

  // Get file mode
  const appendMode = document.querySelector('input[name="file-mode"]:checked').value === 'append';
  ...
  if (editingVault) {
    // Update existing
    vaultData.id = editingVault.id;
    const index = quickCaptureSettings.vaults.findIndex(v => v.id === editingVault.id);
    if (index !== -1) {
      quickCaptureSettings.vaults[index] = vaultData;
    }
  } else {
    // Add new
    vaultData.id = 'vault-' + Date.now();
    quickCaptureSettings.vaults.push(vaultData);
  }

  renderVaultsList();
  closeVaultEditDialog();
});
...
document.getElementById('close-vault-edit-dialog').addEventListener('click', () => {
  closeVaultEditDialog();
});

// Icon Picker
document.getElementById('selected-icon-btn').addEventListener('click', () => {
...
document.addEventListener('click', (e) => {
  ...
  const popup = document.getElementById('icon-picker-popup');
  if (iconPicker && !iconPicker.contains(e.target) && !popup.classList.contains('hidden')) {
    popup.classList.add('hidden');
  }
});

// Browse for vault path
document.getElementById('browse-vault-path-btn').addEventListener('click', () => {
  ipcRenderer.send('browse-vault-path');
});

ipcRenderer.on('vault-path-selected', (event, path) => {
  document.getElementById('vault-path').value = path;
});

// Save Quick Capture Settings
document.getElementById('save-capture-settings-btn').addEventListener('click', () => {
  // Collect settings from UI
  quickCaptureSettings.enabled = document.getElementById('quick-capture-enabled').checked;
  quickCaptureSettings.windowStyle = document.querySelector('input[name="window-style"]:checked').value;
  quickCaptureSettings.clearAfterSend = document.getElementById('clear-after-send').checked;
  quickCaptureSettings.showFilenamePreview = document.getElementById('show-filename-preview').checked;

  // Send to main process
  ipcRenderer.send('update-quick-capture-settings', quickCaptureSettings);

  closeQuickCaptureSettingsDialog();
});

// Cancel Quick Capture Settings
document.getElementById('cancel-capture-settings-btn').addEventListener('click', () => {
  closeQuickCaptureSettingsDialog();
});

document.getElementById('close-capture-settings-dialog').addEventListener('click', () => {
  closeQuickCaptureSettingsDialog();
});

// Settings saved confirmation
ipcRenderer.on('quick-capture-settings-saved', () => {
  console.log('Quick Capture settings saved successfully');
});
