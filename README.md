...
Download `Nthing Setup 1.9.4.exe` from releases and run it. The installer will set up file associations for .md and .markdown files so you can double-click them to open in Nthing.
...
Run `npm run build` to create a Windows installer. It'll show up in `dist/Nthing Setup 1.9.4.exe`. There's also `npm run build:portable` if you want a standalone exe.
